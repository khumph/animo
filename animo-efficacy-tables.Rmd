...
make_table(blood_results, blood_labels, df = animo, 
...
macro_labels <- tibble(
  ...
  ltpa = "LTPA<sup>c</sup> (minutes/week)",
  `log(kcals_per_day)` = "Average caloric intake (kcal/day)<sup>d</sup>"
)
```
...
```{r macro mean table}
make_table(macro_results, macro_labels, df = animo, 
           caption = "Table XX: measures.",
           tfoot = "\n<sup>c</sup>Lesiure time physical activity.\n<sup>d</sup>Differences in caloric intake are percent differences")
```
