...
pacman::p_load(magrittr, tidyverse, plotly, Gmisc)
select <- dplyr::select
```

```{r load data}
animo <- read.csv("./data-processed/all.csv", stringsAsFactors = F)
screen <- read.csv("./data-raw/screen.csv", stringsAsFactors = F)
```

```{r format data}
animo <- animo %>% mutate(group = factor(group, 0:1, c("WLC", "GCSWLI")))
```
...
num_eligible <- sum(screen$eligible_esf == "Yes")
num_ineligible <- sum(screen$eligible_esf == "No")
...
The two participants who weren't randomized were removed in everything below.
...
```{r}
animo <- animo %>%
  # mutate_at(vars(starts_with("chng"), progress_why_tss), funs(as.double)) %>%
  mutate(
